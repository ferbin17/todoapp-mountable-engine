...
module Todoapp
  class UsersController < ApplicationController
    ...
    def login
      @user = Todoapp::User.new
      if request.post?
        @user = Todoapp::User.new(user_params)
        if @user.authenticate
          user = Todoapp::User.active.where("email_id = ?", @user.email_id).first
    ...
    def sign_up
      ...
      if request.post?
        @user = Todoapp::User.new(user_params) 
        check_user_exists = Todoapp::User.find_by_email_id(@user.email_id).present?
    ...
    def sent_otp
      ...
      if !(session[:otp]["sent_time"].present?) || time_diff.to_i > 120
        ...
        @user = Todoapp::User.new(email_id: params[:email_id]) unless @user.present?
    ...
    def forgot_password
      @user = Todoapp::User.new
      if request.post?
        @user = Todoapp::User.find_by_email_id(user_params[:email_id])
        if @user.present?
        ...
        if params[:reset_password_token].present?
          @user = Todoapp::User.find_by_reset_password_token(params[:reset_password_token])
          if @user.present?
